...
+!serve : isAbleToServe & detected(person) <- .findall(O, detectedAndCanServeWith(O), L1)
						      				  ...
						      				  .findall(O, detectedAndCanServeIn(O), L2)
			                   				  ...
			                   				  sglib.groundSymbol(detected(ObjToServeWith), ObjToServeWithGrounds); 
											  if (.length(ObjToServeWithGrounds) > 0) {
											  	sglib.groundSymbol(detected(ObjToServeIn), ObjToServeInGrounds); 
											  	if (.length(ObjToServeInGrounds) > 0) {
													.print("It will serve with ", ObjToServeWith, " in ", ObjToServeIn)
							  				  		!serveWith(ObjToServeWith, ObjToServeIn)											  		
											  	} else {
											  		.print("Agent has detected(", ObjToServeIn,") to serve with, but it's not grounded. Therefore, it will not be considered valid to be used.")
											  	}
											  } else {
												.print("Agent has detected(", ObjToServeWith,") to serve in, but it's not grounded. Therefore, it will not be considered valid to be used.")
											  }.
...
+!refreshPerceptionsAndCheckSuccessful <- .print("The agent will verify that it was successful...");
										  ...
										  .print("Verfied on environment. Checking successful based on beliefs...");
										  !checkSuccessful;
										  -serving.
